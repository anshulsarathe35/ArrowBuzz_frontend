...
const CustomNavLinkList = ({ href, className, isActive, children }) => {
  const linkStyles = "text-[17px] font-medium cursor-pointer list-none hover:text-blue transition-all ease-in-out";
  const activeClass = isActive ? "text-blue" : "";
...
const CustomNavLink = ({ href, className, isActive, children }) => {
  const linkStyles = "text-[17px] font-medium cursor-pointer list-none hover:text-blue transition-all ease-in-out";
  const activeClass = isActive ? "bg-green_100 text-blue" : "";
...
const PrimaryButton = ({ children, className }) => {
  return (
    <button type="submit" className={`${className} text-white bg-blue-600 font-medium rounded-full text-lg px-16 py-3 hover:bg-slate-600 transition ease-in-out`}>
      {children}
    </button>
  );
};
...
export const commonClassNameOfInput = "w-full p-4 text-sm text-gray-900 border border-gray-200 focus:ring-blue focus:border-blue outline-none";
